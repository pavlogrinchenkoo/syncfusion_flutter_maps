...
Offset _getPointAlongLineWithCap(Offset start, Offset end, double capRadius,
...
void _drawInvertedPath(PaintingContext context,
    Path path,
  ...
  if (kIsWeb) {
    ...
    context.canvas..drawPath(
        ...
        fillPaint)..drawPath(path, strokePaint);
  }
}

Color _getHoverColor(Color? elementColor, Color layerColor,
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    for (final MapLine line in lines) {
      ...
      if (line.dashArray != null) {
        ...
        _drawDashedLine(context.canvas, line.dashArray!, paint, path, null,
    ...
    for (final MapArc arc in arcs) {
      ...
      if (arc.dashArray != null) {
        ...
        Path svgPath = Path()
          ..reset()
          ..moveTo(startPoint.dx, startPoint.dy);
        svgPath = _getAnimatedPath(_get1SvgPath(
            Size(boxSize.width, 10),
            svgPath,
            startPoint.dx,
            startPoint.dy,
            endPoint.dx,
        ...
        context.canvas.drawPath(svgPath, paint1);
        // _drawDashedLine(
        //     context.canvas, _dashArray, paint, path, arc.root);
      }
    }
    context.canvas.restore();
  }
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    for (final MapPolyline polyline in polylines) {
      ...
      if (polyline.dashArray != null) {
        ...
        _drawDashedLine(context.canvas, polyline.dashArray!, paint, path, null,
            hasCap ? strokeWidth / 2 : 0);
      } else {
        _drawDashedLine(context.canvas, _dashArray, paint, path);
      }
...
class _RenderMapPolygon extends RenderBox implements MouseTrackerAnnotation {
  ...
  void _updateHoverStrokeColor(Paint paint, MapPolygon polygon,
      ColorTween tween) {
...
class _RenderMapCircle extends RenderBox implements MouseTrackerAnnotation {
  ...
  void _updateHoverStrokeColor(Paint paint, MapCircle circle,
      ColorTween tween) {
    if (_themeData.shapeHoverStrokeColor != Colors.transparent) {
      paint.color = tween.evaluate(_hoverColorAnimation)!;
    } else {
...
Path _get1SvgPath(Size size, Path path, double sx,
    double sy, double ex, double ey, double fr, double sr) {
  Offset startPoint = Offset(sx, sy);
  Offset endPoint = Offset(ex, ey);
  ...
  double dy = (endPoint.dy + 5) - (startPoint.dy + 5);
...
void _drawDashedLine(Canvas canvas, List<double> dashArray, Paint paint,
    Path path, [PictureInfo? image, double capRadius = 0]) {
  bool even = false;
  ...
  if (!even) {
    ...
    if (image != null) {
      // canvas.drawPicture(image.picture);
    }
  }
}
...
bool _liesPointOnLine(Offset startPoint, Offset endPoint, double touchTolerance,
  ...
  path
    ...
    ..lineTo(lineTopRight.dx, lineTopRight.dy)..lineTo(
      lineBottomRight.dx, lineBottomRight.dy)..lineTo(
